...
# helper function to draw and update sprites
...
# helper function to handle group-other collisions
...
# helper function to handle group-group collisions
...
def draw(canvas):
    global time, started, lives, MAX_ROCK, rock_group, score, my_ship, missile_group, explosion_group
...
#    a_missile.update()
    ...
    # Increase score when missile hits a rock
    score += group_group_collide (rock_group, missile_group) * 10
    
    # Increase # of rocks depending on the score
    if score > 500:
        MAX_ROCK = 24
    elif score > 350:
        MAX_ROCK = 18
...
def rock_spawner():
    global rock_group, MAX_ROCK, my_ship, started, asteroid_info
    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
    rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
    rock_avel = random.random() * .2 - .1
    
    if started: 
        if len(rock_group) < MAX_ROCK: # Check that no more than 12 rocks are active at the same time
            if dist(rock_pos, my_ship.get_position ()) > asteroid_info.get_radius () + my_ship.get_radius () + 10: # Avoid rocks spawning near the ship
