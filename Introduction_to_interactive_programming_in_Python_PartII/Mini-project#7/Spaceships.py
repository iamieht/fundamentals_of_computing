...
class Ship:
    ...
    def update(self):
        ...
        # Move forward when thrust is True
        if self.thrust:
            vel = angle_to_vector(self.angle)
            for i in range(2):
                self.vel[i] += vel[i] * 0.10                
        else:                
        # Friction
            self.vel[0] *= (1 - 0.0025)
            self.vel[1] *= (1 - 0.0025)
        
    def increase_angelvel(self):
        self.angle_vel += .05

    def decrease_angelvel(self):
        self.angle_vel -= .05
        
    def update_thrust(self, thrusting):
    ...
    def shoot(self):
        global a_missile
        vel = angle_to_vector(self.angle)
        missile_position = [self.pos[0] + self.radius * vel[0], self.pos[1] + self.radius * vel[1]]
        missile_velocity = [self.vel[0] + 3 * vel[0], self.vel[1] + 3 * vel[1]]
...
def down(key):
    if key == simplegui.KEY_MAP['left']:
        my_ship.decrease_angelvel()
    elif key == simplegui.KEY_MAP['right']:
        my_ship.increase_angelvel()
